...
export function reactToWebComponent<P extends object>(
  ...
  { tagName, shadow = { mode: 'open' }, attrPrefix = '', observedProps = [], initialProps = {}, styles }: Options = {}
) {
  ...
  class ReactCustomElement extends HTMLElement {
    ...
    constructor() {
      ...
      // Where React renders
      if (shadow) {
        const init: ShadowRootInit = typeof shadow === 'object' ? shadow : { mode: 'open' }
        this._host = this.attachShadow(init)
        // Inject styles
        if (styles) {
          if (Array.isArray(styles) && (this._host as any).adoptedStyleSheets !== undefined) {
            ;(this._host as any).adoptedStyleSheets = [...(this._host as any).adoptedStyleSheets, ...styles]
          } else {
      ...
      this._observer = new MutationObserver((records) => {
        for (const r of records) {
          ...
          const logical = toCamel(attrPrefix ? attr.slice(attrPrefix.length) : attr)
      ...
      for (const attr of Array.from(this.attributes)) {
        ...
        const logical = toCamel(attrPrefix ? name.slice(attrPrefix.length) : name)
