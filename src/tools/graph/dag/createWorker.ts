...
import { ID, UpdateMessage } from './DagMessages'

let worker: Worker

export const createWorker = () => {
  // @ts-ignore
  const workerPath = new URL('./DagWorker.bundle.js', import.meta.url).href
  const worker = createWorkerFromCrossOriginURL(workerPath, true, { name: 'DagWorker' })
  return worker
}
